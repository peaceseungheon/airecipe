...
| 2026-05-30 | AI Provider에 Kimi(Moonshot AI) 어댑터 추가 + 기본 Provider `gemini`→`kimi` 전환 결정 ADR-012 발행 (문서/계약 정합성) | `docs/adr/ADR-012-kimi-moonshot-provider.md`·`docs/adr/ADR-008`(후속 링크)·`docs/SESSION_NOTES.md`(세션 #7)·`CLAUDE.md` | 사용자 요청. Kimi는 OpenAI 호환(api.moonshot.ai/v1, 모델 kimi-k2) → ADR-008 Factory+Adapter에 편입. `openai` SDK + baseURL, env `KIMI_API_KEY`/`KIMI_MODEL`/`KIMI_BASE_URL`. Gemini·Claude 롤백 보존. **응답 계약(`GeneratedRecipe`·`RecommendationItem[]`) 불변** → 미니앱·프론트 무영향. 코드 구현은 recipe-backend 병렬 진행 |
| 2026-05-31 | 백엔드 화면(UI) 계층 전면 제거 — API 전용으로 정리 | `src/app/{page,layout}.tsx`·`globals.css`·`favicon.ico`·`src/app/{auth,my-recipes,recipe}/`·`src/components/`·`src/hooks/`·`public/*.svg`·`src/lib/utils.ts`·`postcss.config.mjs`·`src/lib/auth.ts`(레거시 dead code)·`package.json`(swr/clsx/tailwind-merge/tailwindcss/@tailwindcss/postcss 제거)·`docs/SESSION_NOTES.md` | 화면은 미니앱(airecipe-miniapp)이 담당. 백엔드는 API·Service·Repository·AI만 보유. 서버 코드는 components/hooks 무참조라 안전 제거 — `next build` 통과(API 라우트 5개만 빌드, 화면 0). react/react-dom·@types/react(-dom)은 Next 런타임 필요로 유지. pnpm lockfile에서 UI 패키지 프루닝 완료 |
