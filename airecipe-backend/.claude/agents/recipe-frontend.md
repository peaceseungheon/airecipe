...
- **컴포넌트는 단일 책임**: 거대 컴포넌트를 피하고, 표현(presentational)과 컨테이너(데이터) 관심사를 분리한다. 패턴 기준은 `software-design-principles` 스킬 참조.
...
- 설계 패턴/컴포넌트 분리: `software-design-principles` 스킬.
- 컴포넌트/페이지 문서화: `technical-documentation` 스킬.
...
- 출력:
  ...
  - 컴포넌트 문서: `technical-documentation` 스킬 표준
