...
- **D74 — 사업자 정보는 placeholder + 출시 전 확정 의무.** 법인명·대표자·관할 법원·개인정보 보호책임자·고객센터 채널은 미확정이라 `[ ]` placeholder로 두고, 본문·JSDoc·검수 PENDING에 출시 전 실제 값 확정 의무를 명시. 고객센터는 앱인토스 콘솔 고객센터 채널과 연동.
  - **확정 완료 (2026-06-03):** 서비스 제공자/개인정보처리자 **디지털공방**, 개인정보 보호책임자 **이승헌**, 문의 이메일 **tmdgis19@gmail.com**(보호책임자 연락처 겸 고객센터). 관할 법원은 별도 합의 관할 없이 **「민사소송법」 법정관할** 기준으로 placeholder 제거. 사업자등록번호·대표자 별도 미수령(추후 필요 시 병기). 인앱(`pages/terms.tsx`·`privacy.tsx`)·외부(`legal/terms.html`·`privacy.html`) 양쪽 동기 교체. **법무 검토는 여전히 권고(미완).**
...
- ~~**사업자 정보 확정(D74)**~~ — **완료(2026-06-03).** 디지털공방·이승헌·tmdgis19@gmail.com + 관할 민사소송법 기준으로 인앱·외부 HTML 양쪽 placeholder 교체. 상세는 위 D74 확정 완료 절. (사업자등록번호·대표자는 추후 필요 시 병기)
- **법무 검토** — 생성된 표준 보일러플레이트는 일반 템플릿이므로, 실제 서비스 운영 형태에 맞춘 법무 검토 후 확정 권장. **(여전히 미완)**
