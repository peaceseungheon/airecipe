...
| **D56** | 탭바는 **`/` 와 `/my-recipes` 두 화면에만** 렌더. 스택 라우트(`/recipe/generate`·`/recipe/[id]`·`/recipe/recommend`)·`/_404`에는 **렌더하지 않음**(상세/생성/추천은 몰입형 진입, BackButton 헤더로 복귀). | §1.1 — 글로벌 탭바를 끼울 루트 슬롯이 없으므로 "탭이 보여야 하는 화면이 직접 렌더"가 유일·정합한 방법 |
