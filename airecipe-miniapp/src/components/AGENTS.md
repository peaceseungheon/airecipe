...
| `AppInlineAd.tsx` (Phase 4.5 / ADR-022 D86) | 토스 인라인 광고 합성 — `ads.InlineAdSlot` 위임. props `{ slot, theme?, tone?, variant? }`. SDK BannerSlotCallbacks 미노출. **`AdErrorBoundary`로 감싸 렌더 실패 시 배너만 숨김(앱 크래시 방지)**. dev에서 placeholder, staging/prod(+ADS_ENABLED)에서 실 광고 | 11-ADS §11.4, ADR-014 D26·D31·D33, ADR-022 D86 |
| `TopAdBanner.tsx` (ADR-022 rev.2) | **전 화면 상단 고정 배너** — 각 화면이 `PageNavbar` 아래 `<TopAdBanner slot="..." />` 1줄 마운트(화면-내 마운트, ⚠ `_app.tsx` 루트 마운트 금지 — rev.1 크래시). `ads.isEnabled()` false면 `null`(공간 0·회귀 0). `<AppInlineAd>` 위임 — SDK 직접 import 0건 | 11-ADS §11.5.1, ADR-022 D84·D87·D88 |
