...
`airecipe-miniapp`(앱인토스 RN+Granite+TDS 미니앱)을 개발하는 에이전트 팀을 조율하여 동작하는 코드와 철저한 문서를 생성하는 통합 스킬. 본 저장소는 **미니앱 클라이언트만** 담고 백엔드는 별 저장소(`AIReceipe`)의 Next.js+Vercel을 HTTPS 호출한다.
...
|------|-------------|------|----------|------|
| `miniapp-architect` | miniapp-architect (커스텀) | 아키텍처·SSOT 인용·ADR·문서 총괄 | software-design-principles, technical-documentation, granite-rn-development, appsintoss-publish-checklist | `_workspace/01_architect_*.md`, `docs/adr/`, `AGENTS.md` |
| `miniapp-api-client` | miniapp-api-client (커스텀) | 백엔드 호출 단일 경로·useTossUserId·zod·SSE 어댑터 | granite-rn-development, software-design-principles, technical-documentation | `src/services/`, `src/hooks/useTossUserId.ts`, `src/lib/zod/` |
| `miniapp-frontend` | miniapp-frontend (커스텀) | Granite pages·TDS RN·SSE 소비·낙관적 업데이트 | granite-rn-development, software-design-principles, technical-documentation, appsintoss-publish-checklist | `pages/`, `src/components/`, `src/_app.tsx` |
| `miniapp-qa` | miniapp-qa (커스텀, general-purpose 기반) | 통합 정합성·TDS 실재성·검수 정책 | integration-coherence-qa, appsintoss-publish-checklist | `_workspace/03_qa_report.md` |
