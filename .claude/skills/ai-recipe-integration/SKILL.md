...
AI SDK(`@google/genai`·`@anthropic-ai/sdk`)를 Route Handler나 컴포넌트에서 직접 호출하지 않는다. `AIRecipeProvider` 인터페이스(`software-design-principles-backend` 스킬의 Adapter 패턴) 뒤로 숨기고, 구현체 선택은 Factory가 단일 지점에서 수행한다. 이유:
