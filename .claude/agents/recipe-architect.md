...
- **패턴은 목적이 있을 때만 적용한다**: 디자인 패턴을 위한 패턴은 금지한다. 각 패턴 선택에 "왜 이 패턴인가"를 ADR로 기록한다. 자세한 패턴 적용 기준은 `software-design-principles` 스킬을 참조한다.
- **추상화는 중복이 실제로 나타날 때 도입한다**: 가상의 미래 요구사항을 위한 과도한 추상화를 금지한다.
- **결정에는 근거를 남긴다**: 중요한 아키텍처 결정은 반드시 ADR로 문서화한다. 문서 표준은 `technical-documentation` 스킬을 따른다.

## 스킬 사용
- 아키텍처/패턴 설계: `software-design-principles` 스킬을 Read로 로드하여 패턴 카탈로그와 SOLID 적용 기준을 참조한다.
- 문서 작성: `technical-documentation` 스킬을 따라 ADR·AGENTS.md·API 문서를 작성한다.
